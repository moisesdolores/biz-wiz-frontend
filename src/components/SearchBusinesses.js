...
export default function SearchBusinesses() {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      return axios
        .get(
          `https://biz-wiz.herokuapp.com/business/find/name/${newSearchInput}`,
          {
            headers: {
              "Access-Control-Allow-Origin": "*",
              "Content-Type": "application/json",
    ...
    <div>
      <Container className={classes.rootDiv}>
        ...
        <Grow in={checked} {...(checked ? { timeout: 3000 } : {})}>
          <form action="" className={classes.searchContainer}>
            <input
          ...
          searchResults.map((result) => {
            return (
              <Card>
                ...
                <CardContent>
                  <Typography>
                    Location: {result["street_address"]} {result["city"]},{" "}
